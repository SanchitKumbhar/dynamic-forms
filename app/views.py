...
def AnswerAPI(request,id):
    jsondata=json.loads(request.body)
    print(jsondata)
    eventinstance=FormInfo.objects.get(pk=id,user=request.user)
    Answers.objects.create(answer=jsondata,event=eventinstance,username="Test")
...
def  Responses(request,id):
    return render(request,"responses.html",{'responses':Answers.objects.filter(event=id).filter(user=request.user)})
